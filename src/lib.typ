...
#let _validate-direction(direction) = {
  return if type(direction) == str {
    if direction == "left" { 0 } else if direction == "bottom-left" or direction == "lower-left" { 1 } else if (
      direction == "bottom" or direction == "down"
    ) { 2 } else if direction == "bottom-right" or direction == "lower-right" { 3 } else if direction == "right" {
      4
    } else if direction == "top-right" or direction == "upper-right" { 5 } else if (
      direction == "top" or direction == "up"
    ) { 6 } else if direction == "top-left" or direction == "upper-left" { 7 } else { 0 }
...
#let get-language-index(lang) = {
  if lang == "en" { 0 } else if lang == "de" { 1 } else { 0 }
}
...
#let fire-sign(
  ...
  index = if index < 10 { "00" + str(index) } else if index < 100 { "0" + str(index) }
...
#let fire-arrow(
  ...
  box(
    rotate(direction, image(image-path, alt: "arrow pointing at " + direction-name + ", dangerous condition", ..args)),
    inset: inset,
  )
}
...
#let emergency-arrow(
  ...
  direction = int(direction / 2) * 90deg
  box(
    rotate(direction, image(image-path, alt: "arrow pointing at " + direction-name + ", safe condition", ..args)),
    inset: inset,
  )
}
...
#let iso-7010(
  ...
  if category == "f" { fire-sign(index, inset: inset, ..args) } else if category == "w" {
    warning-sign(index, ..args)
  } else if category == "p" { prohibited-actions-sign(index, ..args) } else if category == "e" {
    emergency-sign(index, inset: inset, ..args)
